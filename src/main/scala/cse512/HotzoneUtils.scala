...
object HotzoneUtils {
  ...
  def ST_Contains(queryRectangle: String, pointString: String ): Boolean = {
	...
	val minX =  if(rectangle(0) < rectangle(2)) rectangle(0) else rectangle(2)
    val minY =  if(rectangle(1) < rectangle(3)) rectangle(1) else rectangle(3)
    val maxX =  if(rectangle(0) < rectangle(2)) rectangle(2) else rectangle(0)
    val maxY =  if(rectangle(1) < rectangle(3)) rectangle(3) else rectangle(1)
